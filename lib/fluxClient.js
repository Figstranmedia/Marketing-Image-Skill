...
export class FluxClient {
  ...
  async generateImages(prompts, outputFolder) {
    ...
    for (const promptData of prompts) {
      ...
      try {
        const imageUrl = await this.generateImage(promptData.prompt, promptData.width, promptData.height);
   ...
   */
  async generateImage(prompt, width = 1080, height = 1350) {
    ...
    const prediction = await this.createPrediction(prompt, modelVersion, 3, width, height);
   ...
   */
  async createPrediction(prompt, modelVersion, retries = 3, width = 1080, height = 1350) {
